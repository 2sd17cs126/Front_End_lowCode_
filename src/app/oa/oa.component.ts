...
export class OAComponent {
  ...
  automatic_fetch(){
    this.http
    .post('http://127.0.0.1:8000/automatic_fetch', {
  ...
  invokeExtension(): void {
    this.http
    .post('http://127.0.0.1:8000/open_popup', {
  ...
  enhance(id: string) {
    ...
    this.sleep(3000).then(() => {
      ...
      this.c_name=temp_row['class_name'];
      this.c_f=temp_row['class_functions']
      console.log("c_name");
      console.log(this.c_name);
      console.log(this.c_f);
    });
